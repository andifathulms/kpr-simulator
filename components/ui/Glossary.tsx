...
/**
 * The whole vocabulary, for the front page.
 *
 * `only` narrows it to the words a particular page actually uses. The
 * definitions belong beside the form that uses them — someone arriving on
 * `hitung` from a shared link never passes the home page, and a word defined
 * two clicks away is not defined.
 */
export function Glossary({ locale, only }: { locale: Locale; only?: readonly TermKey[] }) {
  const entries = only ? TERMS.filter((entry) => only.includes(entry.key)) : TERMS
  return (
